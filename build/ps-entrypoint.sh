...
if [ "$1" = 'mysqld' ] && [ -z "$wantHelp" ]; then
	...
	if [ ! -d "$DATADIR/mysql" ]; then
		...
		if [ -n "$CLUSTERSET_PASSWORD" ]; then
			clusterset_dynamic_privs="BACKUP_ADMIN, CLONE_ADMIN, CONNECTION_ADMIN, GROUP_REPLICATION_ADMIN, REPLICATION_SLAVE_ADMIN, REPLICATION_APPLIER, PERSIST_RO_VARIABLES_ADMIN, ROLE_ADMIN, SESSION_VARIABLES_ADMIN, SYSTEM_VARIABLES_ADMIN"
			if [ "$MYSQL_VERSION" == '9.7' ]; then
				clusterset_dynamic_privs="${clusterset_dynamic_privs}, TRANSACTION_GTID_TAG"
			fi
			read -r -d '' clustersetCreate <<-EOSQL || true
				...
				GRANT ${clusterset_dynamic_privs} ON *.* TO 'clusterset'@'%' WITH GRANT OPTION ;
