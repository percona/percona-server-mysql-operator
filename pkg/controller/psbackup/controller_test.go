...
func TestBackupStatusErrStateDesc(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "suspends existing job when cluster is not ready",
			cr:   cr,
			cluster: updateResource(
				cluster.DeepCopy(),
				func(cr *apiv1.PerconaServerMySQL) {
			...
			obj: []client.Object{
				&batchv1.Job{
					ObjectMeta: metav1.ObjectMeta{
						Name:      xtrabackup.JobName(cr),
						Namespace: namespace,
					},
				},
			},
			state:     apiv1.BackupSuspended,
			stateDesc: "suspended due to cluster not being ready",
		},
		{
			name: "resumes suspended job when cluster becomes ready",
			cr: updateResource(cr.DeepCopy(), func(cr *apiv1.PerconaServerMySQLBackup) {
				cr.Status.State = apiv1.BackupSuspended
				cr.Status.StateDesc = "suspended due to cluster not being ready"
...
func TestReconcileBackupJob(t *testing.T) {
	scheme := runtime.NewScheme()
	require.NoError(t, clientgoscheme.AddToScheme(scheme))
	require.NoError(t, apiv1.AddToScheme(scheme))
	...
	cr, err := readDefaultCRBackup("some-name", namespace)
	require.NoError(t, err)
	cr.Spec.ClusterName = "ps-cluster1"
	...
	tests := []struct {
		...
		cluster        *apiv1.PerconaServerMySQL
		job            *batchv1.Job
		initialStatus  apiv1.PerconaServerMySQLBackupStatus
		expectedState  apiv1.BackupState
		expectedDesc   string
		expectedPaused bool
	}{
		{
			name: "suspends running job when cluster is not ready",
			...
			expectedState:  apiv1.BackupSuspended,
			expectedDesc:   "suspended due to cluster not being ready",
			expectedPaused: true,
		},
		{
			name: "resumes suspended job when cluster becomes ready",
			...
			expectedState:  apiv1.BackupStarting,
			expectedDesc:   "",
			expectedPaused: false,
		},
		{
			name: "keeps job suspended without suspended condition",
			cluster: updateResource(cluster.DeepCopy(), func(cluster *apiv1.PerconaServerMySQL) {
				cluster.Status.State = apiv1.StateReady
				cluster.Status.MySQL.State = apiv1.StateReady
			}),
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			r := PerconaServerMySQLBackupReconciler{
				Client: fake.NewClientBuilder().
					WithScheme(scheme).
					WithObjects(cr, tt.cluster, tt.job).
					WithStatusSubresource(cr, tt.cluster, tt.job).
					Build(),
				Scheme: scheme,
			}

			status := tt.initialStatus
			require.NoError(t, r.reconcileBackupJob(ctx, cr, tt.cluster, &status))

			job := new(batchv1.Job)
			require.NoError(t, r.Get(ctx, client.ObjectKeyFromObject(tt.job), job))
			require.NotNil(t, job.Spec.Suspend)
			assert.Equal(t, tt.expectedPaused, *job.Spec.Suspend)
			assert.Equal(t, tt.expectedState, status.State)
			assert.Equal(t, tt.expectedDesc, status.StateDesc)
		})
	}
}
