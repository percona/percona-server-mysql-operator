...
func reconcileSSLSecrets(ctx context.Context, cl client.Client, cr *v2.PerconaServerForMySQL) error {
	if err := createSSLCerts(ctx, cl, cr); err != nil {
...
func createSSLCerts(ctx context.Context, cl client.Client, cr *v2.PerconaServerForMySQL) error {
	sslSecret := &corev1.Secret{}
	nn := types.NamespacedName{Name: cr.Spec.SSLSecretName, Namespace: cr.Namespace}
	err := cl.Get(ctx, nn, sslSecret)
	if err == nil {
		return nil
	...
	if err := createSSLManually(ctx, cl, cr); err != nil {
...
func createSSLManually(ctx context.Context, cl client.Client, cr *v2.PerconaServerForMySQL) error {
	// TODO: DNS suffix
	hosts := []string{
		"*." + cr.Name + "-mysql",
	...
	if err := k8s.SetControllerReference(cr, &secretObj, cl.Scheme()); err != nil {
		return errors.Wrapf(err, "set controller reference to %s/%s", secretObj.Kind, secretObj.Name)
	}
	if err := cl.Create(ctx, &secretObj); err != nil {
